...
void
MpTcpSocketBase::SetSegmentSize (uint32_t size)
{
	m_segmentSize=size;
}
...
MpTcpSocketBase::DupAck (const TcpHeader& t, uint32_t count)
{
 ...
 if (count == m_retxThresh && !m_inFastRec)
    { // triple duplicate ack triggers fast retransmit (RFC2581, sec.3.2)
      ...
      m_cwd = m_ssThresh + 3 * m_segmentSize;
      m_inFastRec = true;
      NS_LOG_INFO ("Triple dupack. Reset cwnd to " << m_cwd << ", ssthresh to " << m_ssThresh);
      DoRetransmit ();
    }
  else if (m_inFastRec)
    { // In fast recovery, inc cwnd for every additional dupack (RFC2581, sec.3.2)
      m_cwd += m_segmentSize;
      NS_LOG_INFO ("Increased cwnd to " << m_cwd);
      SendPendingData ();
    };
}
...
MpTcpSocketBase::SendPendingData ()
{
  ...
  while ( ! sendingBuffer->Empty() )
  {
      ...
      if(sFlow->state == ESTABLISHED)
      {
        ...
        size = std::min (size, m_segmentSize);              //And no more than maximum segment size.  //Pablo UC
//        NS_LOG_UNCOND("SIze " << size);
        Ptr<Packet> pkt = sendingBuffer->CreatePacket(size);
        m_totBytes+=size;
//        NS_LOG_UNCOND( Simulator::Now().GetSeconds()<<" "<<"Tot: " << m_totBytes << " Size: " << size);
        if(pkt == 0)
            break;
...
//    uint64_t lDelay = getPathDelay(sFlowIdx);
    ...
    switch ( m_algoCC )
    {
        case Uncoupled_TCPs:
            ...
            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds()<< " GlobalThroughput "<<getGlobalThroughput()<< " Efficacity " <<  getConnectionEfficiency() << " delay "<< getPathDelay(sFlowIdx) << " Uncoupled_TCPs" );
            break;
        case Linked_Increases:
            sFlow->cwnd  = std::max (cwnd  / 2, 1.0);
            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " GlobalThroughput "<<getGlobalThroughput()<< " Efficacity " <<  getConnectionEfficiency() << " delay "<< getPathDelay(sFlowIdx) <<" alpha "<< alpha << " Linked_Increases");
            break;
        case RTT_Compensator:
            sFlow->cwnd  = std::max (cwnd  / 2, 1.0);
            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " GlobalThroughput "<<getGlobalThroughput()<< " Efficacity " <<  getConnectionEfficiency() << " delay "<<getPathDelay(sFlowIdx) <<" alpha "<< alpha << " RTT_Compensator");
            break;
        case Fully_Coupled:
            sFlow->cwnd  = std::max (cwnd - totalCwnd / 2, 1.0);
            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " GlobalThroughput "<<getGlobalThroughput()<< " Efficacity " <<  getConnectionEfficiency() << " delay "<<getPathDelay(sFlowIdx) <<" alpha "<< alpha << " Fully_Coupled");
            break;
        default:
            sFlow->cwnd  = 1;
            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<<sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " GlobalThroughput "<<getGlobalThroughput()<< " Efficacity " <<  getConnectionEfficiency() << " delay "<<getPathDelay(sFlowIdx) << " default");
            break;
    }
...
MpTcpSocketBase::ProcessAction (uint8_t sFlowIdx, Actions_t a)
{
    ...
    ////Pablo UC// CALL TO TRACE WINDOW
    if (!m_mpWindowTraceCallback.IsNull())
    {
  	  m_mpWindowTraceCallback (sFlowIdx,sFlow->cwnd, m_localAddress, sFlow->rtt->GetCurrentEstimate().GetSeconds());
    }
    ////Pablo UC
    return result;
}
...
//                            NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" Moving cwnd from " << sFlow->cwnd << " to " << sFlow->savedCWND << " Throughput "<<(sFlow->cwnd * sFlow->MSS * 8)/sFlow->rtt->est.GetSeconds()<< " GlobalThroughput "<<getGlobalThroughput());
...
MpTcpSocketBase::IsThereRoute (Ipv4Address src, Ipv4Address dst)
{
  ...
  assert (ipv4->GetRoutingProtocol () != 0); //Pablo UC
...
//    double rtt = sFlow->rtt->GetCurrentEstimate().GetSeconds();
    NS_LOG_WARN (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "
    		...
    		<<" Moving cwnd from " << cwnd << " to " << sFlow->cwnd <<" Throughput "<<(sFlow->cwnd * sFlow->MSS * 8)/sFlow->rtt->GetCurrentEstimate().GetSeconds()
...
////Pablo UC
void
MpTcpSocketBase::SetMpWindowTraceCallback (MpWindowTraceCallback callback)
{
	m_mpWindowTraceCallback = callback;
}
////End Pablo UC
