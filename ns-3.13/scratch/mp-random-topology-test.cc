...
//	  LogComponentEnable("MpTcpOnOffApplication", LOG_ALL);
...
//	  LogComponentEnable("UdpEchoClientApplication", LOG_LEVEL_INFO);
	...
	//Configuration file
	string configuration = "mp-random-scenario";
	string scenario;
...
//	EnableLogging ();
	...
	cmd.AddValue ("Values" , "Tell the application if you pass values", Values);
	cmd.AddValue ("Configuration", "Scenario configuration file (located in src/scenario-creator/config)", configuration);
	cmd.AddValue ("Scenario", "Node configuration file (located in src/scenario-creator/config)", scenario);
	cmd.AddValue ("Static", "File which contains the routing table", routingTableFile);
	cmd.AddValue("Fer", "General Frame Error Rate value", fer);
	cmd.AddValue("RunOffset", "Offset of the current simulation", runOffset);
	...
	for (runCounter = 1; runCounter <= GetNumberOfSimulations(configuration); runCounter ++)
	{
		...
		config->m_fer=fer;
		config->m_runOffset=runOffset;

		if (Values)
		{
			config->SetStaticRoutingFileName(routingTableFile);
			config->SetScenarioFileConfig(scenario);
		}
		...
		//Print the initial simulation time
		sprintf(output, "%2d:%2d:%2d - Simulation started", current->tm_hour, current->tm_min, current->tm_sec);
		printf("%s\n", output);
		...
		sprintf(output, "%2d:%2d:%2d - Run %d - %d/%d (FER = %f)", current->tm_hour, current->tm_min, current->tm_sec, runCounter + config->GetRunOffset (),
		config->GetProprietaryTracing () -> GetCorrectPackets (), config->GetProprietaryTracing ()->GetTotalPackets (),
		(double) ((double) config->GetProprietaryTracing() -> GetTotalPackets() -
		(double) config->GetProprietaryTracing() -> GetCorrectPackets()) / (double) config->GetProprietaryTracing() -> GetTotalPackets() );
		printf("%s\n", output);

		if (config->GetTracing())
			propTracing->CloseTraceFile();
		if (config->GetTransportTracing())
			propTracing->CloseMpTcpTraceFile();
		if (config->GetTransportShortTracing())
			propTracing->CloseTransportShortTraceFile();
		if (config->GetWindowTracing())
			propTracing->CloseMpTcpWindowTraceFile();
...
//	NS_LOG_FUNCTION_NOARGS();
	...
	NS_ASSERT_MSG (config.LoadConfig (config.SetConfigFileName("/src/scenario-creator/config/", fileName)) >= 0, "Could not open " << "/src/scenario-creator/config/" <<fileName);
